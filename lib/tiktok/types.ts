...
export interface TikTokOAuthTokens{accessToken:string;accessTokenExpiresAt:number;refreshToken:string;refreshTokenExpiresAt:number;openId:string;sellerName?:string;sellerBaseRegion?:string;userType:number;grantedScopes:string[]}
...
export interface NormalizedBestseller{externalId:string;ranking:number;name?:string;gmv:GmvRange;soldCount?:number;price?:number;creatorCount?:number;videoCount?:number;reviewCount?:number;rating?:number;creatorExternalId?:string;productExternalId?:string;views?:number;rawPayload:unknown}
